...
AUTH = (os.environ.get("MAYAN_APP_USER_NAME"),
        os.environ.get("MAYAN_APP_USER_PASS"))
...
alt_policy_no_metatype_id = mayan_database.get_metatype_by_name(
    "alt_policy_no")
...
def alter_policy_number(policy_number):
    ...
    logger.info(
        f"Updating policy_number {policy_number} to {new_policy_number}")
...
def add_to_cabinet(document_id, client_id):
    ...
    if not result:
        ...
        result = requests.get(
            f"http://{mayan_app_host}/api/cabinets/{cabinet_id}/documents/", auth=AUTH)
        request_raise_exception(
            result, f"Could not fetch cabinet {client_id}'s docs'. code => {result.status_code}")
        ...
        result = requests.post(f"http://{mayan_app_host}/api/cabinets/{cabinet_id}/documents/",
                               data=data, auth=AUTH)
        request_raise_exception(
            result, f"Did not add doc to cabinet: {client_id}. code => {result.status_code}")
...
def current_document_state(request, document_id):
	...
	# date = datetime.date.fromtimestamp(transition.datetime)
	# date.day, .weekday() > 4
    transition.datetime = transition.datetime + delta
    context = {
